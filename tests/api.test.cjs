...
test('API 部署解析、SDS 與角色回應回傳各自結構',async()=>{let r=await call({mode:'deployment_parse'},true,true,'{"vehicles":[],"crews":[],"hoses":[]}');assert.deepEqual(r.data.plan.vehicles,[]);r=await call({mode:'sds_extract'},true,true,'{"productName":"來源產品","sections":{"5":"原文未提供"}}');assert.equal(r.data.sds.sections['5'],'原文未提供');r=await call({mode:'simulation_role'},true,true,'{"text":"請確認任務位置"}');assert.equal(r.data.message.text,'請確認任務位置');});
test('API 拒絕模型格式錯誤，不回傳假成功',async()=>{const r=await call({mode:'sds_extract'},true,true,'{"productName":"沒有章節"}');assert.equal(r.code,502);});
