...
function local(raw,roster){
 ...
 for(const line of clauses){let handled=false;
  ...
  if(/正面.*第一面/.test(line)){add('firstSide',{text:'現場正面為第一面',evidence:line});handled=true;}
  ...
  if(/水源.*(?:缺乏|缺芝|不足|缺之)|待確認|預計|尚未|未到|不宜|不要|不是|受傷|失聯|撤離|休息|PAR/.test(line)){add('note',{text:line,evidence:line});continue;}
  ...
  for(const v of vehicles){add('vehicle',{...v,face:/移至|移到|停在|部署在/.test(line)?(line.match(/第[一二三四]面/)||[])[0]||'':'',evidence:line});handled=true;}
  ...
  else if(vehicles.length>1&&/供水給|供水至|接至/.test(line)){add('hose',{...vehicles[0],number:1,target:vehicles[1].vehicle,task:'供水水線',evidence:line});handled=true;}
  if(!people.length&&!vehicles.length&&mentioned.length===1&&face){add('crew',{unit:mentioned[0],face,task:(line.match(/搜救|內攻|滅火|警戒|供水|待命/)||[])[0]||'',evidence:line,uncertainty:/不宜|不直|不先|未|準備|將/.test(line)?'請核對這是已執行部署還是預定動作':''});handled=true;}
 ...
 return {correctedText:normalized.text,corrections:normalized.corrections,items};
}
function sanitize(raw,source,roster){
 ...
 const items=raw.items.map((x,index)=>{
  ...
  if(!evidence||!normalized.text.includes(evidence))result.uncertainty='請核對此項；AI 沒有提供可對照的原句';
  if(result.number!==null&&!new RegExp('(^|[^0-9])'+result.number+'([^0-9]|$)').test(evidence))result.uncertainty='請核對數量；原句未明確出現此數字';
  if(result.kind==='crew'&&result.unit&&result.number!==null){const pattern=new RegExp(result.unit+'(?:分隊)?[^，；、。]{0,12}?'+result.number+'(?:名|位|個)?人');if(!pattern.test(evidence))result.uncertainty=result.uncertainty||'請核對這個人數是否屬於此分隊';}
 ...
 return {correctedText:text,corrections:normalized.corrections,items};
}
...
function compile(items,state,caseData,roster,options={}){
 ...
 for(const i of selected.slice().sort((a,b)=>order[a.kind]-order[b.kind])){
  ...
  if(['crew','vehicle','hose'].includes(i.kind)){
   ...
   if(i.kind==='crew'){
    ...
    intent={kind:'crew',key:i.id,line:i.evidence||'',unit:i.unit,brigade,group:i.group||'',count:num===null?undefined:num,mode:i.quantityMode||'set',face:i.face||undefined,task:i.task||undefined,targetId:i.targetId||undefined};
    ...
    if(vehicle){
     ...
     intent={kind:'hose',key:i.id,line:i.evidence,source:vehicle,target,count:num,mode:i.quantityMode||'set',rewire:/改接/.test(i.text||i.evidence)};
    }
    else{
...
root.FCIntake29={schema,kinds,labels,number,normalize,local,sanitize,item,compile,assertCaseFresh,caseKeys};if(typeof module!=='undefined')module.exports=root.FCIntake29;
