...
async function setupIdentity28(){
 ...
 if(!/^[\w.-]+\.apps\.googleusercontent\.com$/.test(clientId)){$('authMethod28').textContent='尚未設定 Google 快速登入；請開啟下方「登入設定與檢查」。目前使用備援登入。';return;}
 try{
...
function authErrorMessage28(err){
 ...
 if(/unauthorized-domain|invalid-oauth-client/.test(code))return '此網址尚未加入 Google／Firebase 登入設定，請開啟下方「登入設定與檢查」，依步驟補上正式網域。';
